...
- [ ] confirmar a CSP no host que efetivamente serve `nefroquest.com` — **auditado em 22/08/2026: não existe CSP alguma**, nem header nem `<meta>`. GitHub Pages não define header de resposta; a única via é `<meta http-equiv>` no HTML, e adicionar uma exige inventariar as origens em uso (Supabase, Sentry, Turnstile, Google Fonts, Mercado Pago) para não quebrar o app. Vira etapa própria.
