...
| REF-6 | Referências das perguntas com artigo completo desbloqueável no baú | Cada referência de `data/refs.js` (as que aparecem como "Evidência Científica" dentro das perguntas) deve ter uma versão de artigo completo associada (resumo, abstract ou texto curado). Ao abrir um baú durante o jogo, uma dessas versões é revelada e fica disponível para consulta no Grimório. Enquanto não desbloqueada, a referência aparece travada (🔒) no Grimório — o jogador sabe que existe, mas não pode ler. Discutir antes de executar: (1) formato do conteúdo do artigo — abstract, resumo em PT-BR, ou texto curado? (2) como selecionar qual referência o baú revela — aleatório, por eixo temático da questão respondida, ou sequencial? (3) `refsDB` passa a ser totalmente bloqueado até baú, ou permanece com preview básico (título/autores) e baú libera o conteúdo completo? | ⏳ Pendente — discutir antes de executar |
