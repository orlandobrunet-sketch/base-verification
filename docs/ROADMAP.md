...
| **sentry** (seer + skills + MCP) | Triagem de erros em produção, SDK setup, alertas | Sentry já está inline no `index.html`. Usar `seer` para perguntar "quais erros mais afetam usuários agora", configurar alertas (E7/observabilidade) e investigar erros silenciosos de sync Supabase | Revisar issues abertas no Sentry e priorizar correções | 🔴 |
| **claude-api** (skill) | Referência viva de modelos/preços/tool-use Anthropic | Revisar as edge functions `ai-mentor` e `ai-diagnosis`: confirmar `model id` atual do Haiku, prompt caching para baratear quota, e migração de modelo | Auditar custo/modelo das 2 edge functions de IA | 🟡 |
| **vercel** (skills + CLI) | Deploy, env vars, functions, firewall, performance | Hoje deploy é GitHub Pages + Vercel. Usar para gerir env vars das edge functions, e o `vercel-firewall`/BotID como base do **E9** (proteção dos endpoints de IA contra abuso) | Mapear env vars e avaliar rate-limit/BotID nos endpoints IA | 🟡 |
...
| **playwright** (MCP) | Dirige o browser p/ E2E e auditoria real | Rodar as 9 suites E2E existentes contra mudanças; **auditar de verdade** performance/contraste (validar a auditoria que fiz à mão hoje com Lighthouse/axe); cobrir fluxos sem teste (paywall, ritual de iniciação) | Validar a tela de Resultados no browser + cobrir paywall | 🔴 |
