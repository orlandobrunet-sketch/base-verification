...
| **playwright** (MCP) | Dirige o browser p/ E2E e auditoria real | Rodar as 9 suites E2E existentes contra mudanças; **auditar de verdade** performance/contraste (validar a auditoria que fiz à mão hoje com Lighthouse/axe); cobrir fluxos sem teste (paywall, ritual de iniciação) | **Parcial (v12.01)** — validação browser ponta a ponta (landing→convidado→nova jornada→dificuldade→Glomerulus→jogo): grid de equipamentos renderiza com a arte nova, `object-fit:contain` ok nos slots altos (armadura/arma), **0 erros de console**. **Pendente**: rodar as 9 suites E2E, Lighthouse/axe automatizado, cobrir paywall + ritual | 🟡 |
