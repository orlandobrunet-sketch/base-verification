...
**Substituição dos cinco selos:** nova rodada específica de imagens, cache offline e memória entre jornadas com 13 passes, 1 skip previsto e nenhuma retentativa. A primeira tentativa desta rodada foi interrompida porque o servidor local estava encerrado; a medição válida ocorreu após reiniciá-lo. A CI completa do primeiro commit do PR #800 também passou. O comando de integração automática foi aceito como merge imediato pelo GitHub: a suite completa do commit com os cinco selos ainda estava em andamento na integração. Quality Gates e Smoke passaram; o resultado da suite completa deve ser acompanhado no run `34717610505`. A verificação de produção não equivale à aprovação dessa suite.
...
## Segunda entrega — apresentação do Grimório (15.06 em preparação)
...
- Esta etapa cobre a apresentação no Dashboard. O acesso legado a partir das perguntas ainda abre o modal: sua migração precisa preservar retorno à questão, itens bloqueados, visão administrativa e sugestão de artigo. Não declarar NQ-04B encerrado antes desse recorte.
- Capturas locais em 1440×900, 1366×768, 390×844 e 320×568, além de texto a 200% no celular. O defeito conhecido do cabeçalho ampliado continua no acabamento global.
