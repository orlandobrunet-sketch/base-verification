...
- **[FEITO 2026-06-26] Deletadas Edge Functions órfãs `swift-function` + `clever-worker`** (achado do item 3). Eram versões antigas inseguras de create-preference/webhook (`clever-worker` concedia premium sem HMAC/idempotência). Removidas via CLI com OK explícito do usuário.
- **[FEITO 2026-06-26] Deployado `create-mp-preference`** — a investigação revelou que o sistema de pagamento estava **meio-migrado**: o frontend chama `create-mp-preference` mas ela **nunca havia sido deployada** (404 em prod) — só o `swift-function` antigo estava. Checkout estava quebrado. Deployado o código hardened do repo (verify_jwt=true, aponta p/ mp-webhook); verificado OPTIONS→200, POST sem auth→401. Conjunto agora consistente (7 funções). **Pendente de validação end-to-end** (criar preferência real com usuário autenticado — fazer junto da virada MP).
- **[NÃO FAZER] Leaked password protection** — descartado: é **feature paga** do Supabase Auth. Não será habilitado.
- **[FEITO 2026-06-26] `is_admin()` — removido o email admin hardcoded.** A função tinha um fallback `OR auth.email() = 'orlandobrunet@gmail.com'` "durante transição". Verificado por SQL que a conta dona tem `app_metadata.is_admin=true` → migration **016** aplicada via MCP (com OK explícito), removendo o fallback. `is_admin()` agora só checa `app_metadata.is_admin` (mantém SECURITY DEFINER + search_path). Confirmado por SQL. (O fato de ser chamável por `authenticated` é non-issue — só retorna o status do próprio chamador.)
...
| 3 | Supabase | 1 | DONE (2026-06-26) | **ÚTIL** | MCP reconectado → advisors rodados. **Achados acionáveis:** (a) 2 Edge Functions não-documentadas e públicas (`swift-function` v15, `clever-worker` v14, `verify_jwt:false`) fora das 7 do CLAUDE.md → investigar/remover; (b) Auth "leaked password protection" DESABILITADA → **não fazer** (feature paga); (c) `is_admin()` email hardcoded → **removido** (migration 016 aplicada). **Intencionais (não-bug):** 3× `rls_policy_always_true` (votes/ratings/error_reasons insert público, by-design), `processed_payments` RLS deny-all. **Baixa prio:** 6 índices não usados, políticas permissivas múltiplas em `access_whitelist`. **Drift:** remoto rastreia só migrations 001-002; 003-015 aplicadas manualmente (schema presente). Projeto ACTIVE_HEALTHY, PG 17.6.1. |
